...
const labels = {
  ...
  actions: {
    ...
    createLabel({ commit }, data) {
      return LabelService.addLabel(data.projectId, data).then(response => {
    ...
    importLabels({ commit }, payload) {
      ...
      reader.onload = e => {
        ...
        for (const label of labels) {
          LabelService.addLabel(payload.projectId, label).then(response => {
            commit("addLabel", response.data);
          });
        }
      };
    ...
    exportLabels({ commit }, payload) {
      ...
      LabelService.getLabelList(payload.projectId)
        .then(response => {
          ...
          link.setAttribute(
            "download",
            `project_${payload.projectId}_labels.json`
          );

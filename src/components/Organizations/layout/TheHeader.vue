<template>
  <v-app-bar app clipped-left>
    ...
    <v-btn v-if="isAuthenticated" text @click="$router.push('/projects')">
      Projects
    </v-btn>
    ...
    <v-btn v-if="!isAuthenticated" outlined @click="$router.push('/auth')">
      Sign in
    </v-btn>
    <v-menu v-if="isAuthenticated" bottom>
      <template v-slot:activator="{ on }">
        <v-btn icon v-on="on">
